# Roadmap: Document RAG REST API — v13.0

## Overview

This roadmap details the phased execution plan for **v13.0: Memory Optimization & Cloud Readiness** — optimizing the backend memory profile and enabling OOM-free cloud deployment.

**Phases: 66–68** | **8 requirements (6 completed, 3 cancelled)** | **Continuing from Phase 65 (v12.0)**
...
**Key changes:**
- `backend/app/core/vectorstore.py`: Replace `HuggingFaceEmbeddings` with a custom/wrapper class wrapping `fastembed.TextEmbedding`.
...
**Key changes:**
- `backend/app/routes/query.py`: Skip `FlashRankRerank` initialization and invocation if `ENABLE_RERANKING` is false.
- `backend/app/core/chunker.py`: Enforce garbage collection blocks inside loop scopes.
- `backend/app/routes/upload.py`: Enforce `MAX_UPLOAD_SIZE_MB` upload limit safeguard.
...
---
*Roadmap archived: 2026-07-13 — v13.0 milestone*
